...
def test_gdn2_import_flags_are_boolean():
    assert isinstance(HAVE_FLA, bool)
    assert isinstance(HAVE_GDN2_KERNEL, bool)
    assert HAVE_GDN2 == (HAVE_FLA and HAVE_GDN2_KERNEL)
...
def _make_gdn2(tp_size=1, cp_size=1, sp=False, allow_neg_eigval=False):
    ...
    config = TransformerConfig(
        ...
        experimental_attention_variant="gated_delta_net_2",
...
def test_get_experimental_attention_variant_module_spec_gdn2():
    config = TransformerConfig(
        hidden_size=256,
        linear_conv_kernel_dim=2,
        linear_key_head_dim=64,
    ...
    spec = get_experimental_attention_variant_module_spec(config=config, backend=backend)

    assert spec.module is GatedDeltaNet2
    assert isinstance(spec.submodules, GatedDeltaNet2Submodules)
    assert spec.metainfo["fuse_input_layernorm"] is True
